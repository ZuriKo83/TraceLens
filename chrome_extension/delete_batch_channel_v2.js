(() => {
  ...
  async function deleteYouTubeBatchOnPage(items, expectedPage) {
    ...
    const targets = items.map((item) => {
      ...
      return {
        ...
        title: clean(locator.title || item.title || "").replace(/^\[실시간 채팅\]\s*/, ""),
    ...
    const deleteRow = async (desc) => {
      ...
      for (let attempt = 0; attempt < 24; attempt += 1) {
        ...
        if (/삭제|deleted|removed/i.test(toast) || !document.contains(desc.row)) {
          return {ok: true, confirmed, verification: toast ? "toast" : "removed"};
        }
        if (!confirmed) {
      ...
      if (!document.contains(desc.row)) return {ok: true, confirmed, verification: "removed_late"};
      return {ok: false, confirmed, verification: "not_verified"};
    };
    ...
    for (let step = 0; step < 700; step += 1) {
      ...
      for (const button of deleteButtons()) {
        ...
        const result = await deleteRow(desc);
        if (result.ok) {
          ...
          outcomes.set(chosen.itemId, {
            item_id: chosen.itemId,
            ok: false,
            reason: "삭제 버튼을 눌렀지만 삭제 완료를 확인하지 못했습니다.",
            diagnostics: {
              stage: "verification",
              expected_page: expectedPage,
              confirmed: result.confirmed,
