(() => {
  ...
  async function runPage(tabId, pageItems, page, progress) {
    const initialized = await invoke(tabId, initializeStrictBatchPageV5, [pageItems, page]);
    if (!initialized?.ok) {
      throw new Error(initialized?.error || "Google 내 활동 삭제 검증 엔진을 초기화하지 못했습니다.");
    }
    ...
    for (let index = 0; index < maxSteps; index += 1) {
      const step = await invoke(tabId, strictPrimaryStepV5);
      if (!step?.ok) throw new Error(step?.error || "삭제 단계 실행에 실패했습니다.");

      if (step.action === "clicked") {
        for (let attempt = 0; attempt < 10; attempt += 1) {
          await sleep(attempt < 2 ? 180 : 260);
          const check = await invoke(tabId, strictAfterClickStepV5, [step.item_id]);
          if (!check?.ok || check.ready) break;
        }
        progress();
        stable = 0;
        previousSignature = "";
    ...
    await sleep(1400);
    const verification = await invoke(tabId, strictBeginVerificationV5);
    if (!verification?.ok) {
      throw new Error(verification?.error || "삭제 결과 전체 검증을 시작하지 못했습니다.");
    }
    ...
    if (verification.count > 0) {
      ...
      noMove = 0;
      for (let index = 0; index < maxSteps; index += 1) {
        const step = await invoke(tabId, strictVerificationStepV5);
        if (!step?.ok) throw new Error(step?.error || "삭제 결과 전체 검증에 실패했습니다.");
        if (step.action === "load_more") {
          stable = 0;
          previousSignature = "";
          noMove = 0;
    ...
    const finalized = await invoke(tabId, strictFinalizeV5);
    if (!Array.isArray(finalized?.outcomes)) {
      throw new Error("Google 내 활동 삭제 결과를 만들지 못했습니다.");
    }
  ...
  async function runBatch(job, explicitConfig, port) {
    ...
    for (const [page, pageItems] of grouped) {
      port.postMessage({
        ...
        total: items.length,
        page,
        message: `${page === "youtube_live_chat" ? "실시간 채팅" : "댓글"} 페이지에서 삭제 후 전체 목록을 다시 검증합니다.`,
      });
        ...
        const pageOutcomes = await runPage(tab.id, pageItems, page, () => {
          ...
          try {
            port.postMessage({
              ...
              total: items.length,
              page,
              message: `${Math.min(observed, items.length)}/${items.length}건 삭제 시도 완료 · 최종 성공은 전체 재검증 후 확정`,
            });
          } catch {
            // UI disconnect handler refunds the unfinished job.
          }
        });
  ...
  function initializeStrictBatchPageV5(items, expectedPage) {
    ...
    const signature = () => scrollers()
      ...
      .join("|");
    const nearBottom = () => scrollers().some((node) => (
      node.scrollHeight - node.clientHeight - (node.scrollTop || 0) <= Math.max(24, node.clientHeight * 0.03)
    ));
    const visibleEndMarker = () => [...document.querySelectorAll("div,p,span")].some((node) => {
      if (!visible(node)) return false;
      const text = clean(node.innerText || node.textContent);
      return text.length < 180 && /더 이상 표시할 콘텐츠가 없습니다|no more content/i.test(text);
    });
    const strictEndReached = () => nearBottom() && visibleEndMarker();
    ...
    reset();
    globalThis.__TRACELENS_DELETE_BATCH_V5 = {
      expectedPage,
      targets,
      outcomes: new Map(),
      clickedIds: [],
  ...
  function strictPrimaryStepV5() {
    const state = globalThis.__TRACELENS_DELETE_BATCH_V5;
    ...
    for (const desc of state.rows()) {
      ...
      state.clickedIds.push(chosen.itemId);
      desc.button.click();
      return {ok: true, action: "clicked", item_id: chosen.itemId};
    }

    const loadMore = state.loadMoreButton();
    if (loadMore && state.strictEndReached()) {
      loadMore.click();
      return {ok: true, action: "load_more", moved: false, done: false};
    }
    if (state.strictEndReached()) return {ok: true, action: "scrolled", moved: false, done: true, signature: state.signature()};
    return {ok: true, action: "scrolled", moved: state.moveDown(), done: false, signature: state.signature()};
  }

  function strictAfterClickStepV5(itemId) {
    const state = globalThis.__TRACELENS_DELETE_BATCH_V5;
    ...
    if (confirm) {
      ...
      return {ok: true, ready: false, confirmed: true};
    }

    const stillVisible = state.rows().some((desc) => state.matchesTarget(target, desc));
    return {ok: true, ready: !stillVisible, confirmed: Boolean(target.confirmed)};
  }

  function strictBeginVerificationV5() {
    const state = globalThis.__TRACELENS_DELETE_BATCH_V5;
    if (!state) return {ok: false, error: "삭제 상태가 없습니다."};
    ...
    state.reset();
    return {ok: true, count: state.clickedIds.length};
  }

  function strictVerificationStepV5() {
    const state = globalThis.__TRACELENS_DELETE_BATCH_V5;
    if (!state) return {ok: false, done: true};
    const verifyTargets = state.clickedIds.map((itemId) => state.targets.get(itemId)).filter(Boolean);
    ...
    if (state.strictEndReached()) {
      state.verifyEndSeen += 1;
      if (state.verifyEndSeen >= 2) {
        state.verificationComplete = true;
        return {ok: true, action: "verified", moved: false, done: true, signature: state.signature()};
      }
    } else {
      state.verifyEndSeen = 0;
    }
  ...
  function strictFinalizeV5() {
      ...
      if (target.status === "clicked") {
        ...
        if (!state.verificationComplete) {
          ...
          state.outcomes.set(target.itemId, {
            ...
            ok: true,
            diagnostics: {
              stage: "verified_absent_complete_scan",
              expected_page: state.expectedPage,
              confirmed: Boolean(target.confirmed),
              source_key: target.sourceKey || null,
              verification_complete: true,
    ...
    return {
      ok: true,
      verification_complete: state.verificationComplete,
