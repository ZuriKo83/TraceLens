(() => {
  ...
  async function executeClick(tabId, items, page) {
    const injected = await chrome.scripting.executeScript({
      target: {tabId},
      func: clickBatchHiddenV8,
      args: [items, page],
    });
  ...
  async function clickWithFocusedFallback(tabId, workerWindowId, originalWindowId, items, page) {
    const hiddenResult = await executeClick(tabId, items, page);
    const retryableIds = new Set(
      (hiddenResult.failures || [])
    ...
    const retryItems = items.filter((item) => retryableIds.has(Number(item.item_id)));
    if (!retryItems.length) return {...hiddenResult, focused_fallback_count: 0};

    await chrome.windows.update(workerWindowId, {focused: true}).catch(() => undefined);
    await sleep(250);
    const focusedResult = await executeClick(tabId, retryItems, page);
    if (originalWindowId) {
      await chrome.windows.update(originalWindowId, {focused: true}).catch(() => undefined);
    }

    const retriedSuccess = new Set((focusedResult.applied_item_ids || []).map(Number));
    return {
      ok: true,
      applied_item_ids: [...new Set([
        ...(hiddenResult.applied_item_ids || []).map(Number),
        ...retriedSuccess,
      ])],
      failures: [
        ...(hiddenResult.failures || []).filter((failure) => !retriedSuccess.has(Number(failure.item_id))),
        ...(focusedResult.failures || []),
      ],
      evidence_by_item: {
        ...(hiddenResult.evidence_by_item || {}),
        ...(focusedResult.evidence_by_item || {}),
      },
    ...
    for (const [page, pageItems] of grouped) {
      ...
      try {
        port.postMessage({
          ...
          total: items.length,
          page,
          message: `${page === "youtube_live_chat" ? "실시간 채팅" : "댓글"} 삭제 창을 뒤에서 실행합니다.`,
        });
        ...
        const clickState = await clickWithFocusedFallback(
          tab.id,
          workerWindow.id,
          originalWindow?.id || null,
          pageItems,
          page
        );
        ...
        port.postMessage({
          ...
          total: items.length,
          page,
          message: clickState.focused_fallback_count
            ? `${clickState.applied_item_ids.length}건 삭제 동작 완료 · ${clickState.focused_fallback_count}건만 짧게 전면 재시도 후 백그라운드 검증 중입니다.`
            : `${clickState.applied_item_ids.length}건 삭제 동작 완료 · 화면 전환 없이 백그라운드에서 최종 검증 중입니다.`,
        });
        ...
        for (const item of pageItems) {
          ...
          outcomes.push(stillPresent ? {
            ...
            ok: true,
            diagnostics: {
              stage: "verified_absent_after_hidden_window_click",
              expected_page: page,
              snapshot_complete: true,
              click_evidence: clickState.evidence_by_item?.[String(itemId)] || null,
              focused_fallback_count: clickState.focused_fallback_count || 0,
            },
          });
        }
      } catch (error) {
  ...
  async function clickBatchHiddenV8(items, expectedPage) {
