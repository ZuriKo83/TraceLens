(() => {
  ...
  runExtractor = async function(tabId, platform, activityType, ownershipScope = "self_activity", accountContext = null) {
    ...
    try {
      previousZoom = await chrome.tabs.getZoom(tabId).catch(() => null);
      await chrome.tabs.setZoom(tabId, 0.5).catch(() => undefined);

      for (let step = 0; step < 1600 && collected.size < 5000; step += 1) {
        scanSteps = step + 1;
        const injected = await chrome.scripting.executeScript({
          target: {tabId},
          func: collectYouTubeCommentStep,
          args: [accountContext?.accountLabel || null, step === 0],
        });
        const state = injected?.[0]?.result;
        if (!state?.ok) {
          throw new Error(state?.message || "YouTube 댓글 활동 페이지를 읽지 못했습니다.");
        }
        ...
        if (state.clicked_load_more) loadMoreClicks += 1;
        const signature = `${collected.size}|${state.signature || ""}|${state.clicked_load_more ? 1 : 0}`;
        stable = added > 0 || signature !== previousSignature ? 0 : stable + 1;
        previousSignature = signature;

        if (state.end_reached && !state.loading && !state.load_more_present) {
          endSeen += 1;
          if (endSeen >= 3 && stable >= 1) {
        ...
        if (!state.moved && !state.loading && !state.load_more_present && stable >= 16) break;
      }
    } finally {
      ...
      if (Number.isFinite(previousZoom)) {
        await chrome.tabs.setZoom(tabId, previousZoom).catch(() => undefined);
      }
    }
    ...
    return {
      platform: "youtube",
      source_url: "https://myactivity.google.com/page?page=youtube_comments&hl=ko",
      scan_scope: "comment",
      snapshot_complete: snapshotComplete,
      status: items.length || snapshotComplete ? "success" : "partial",
      message: items.length
        ? `YouTube 댓글 ${items.length}개를 확인했습니다. 원문 링크 ${linkedCount}개 확인${unlinkedCount ? `, ${unlinkedCount}개 미노출` : ""}. ${snapshotComplete ? "전체 목록 확인 완료." : "일부 목록만 확인됨."} 고속 스캔 ${scanSteps}회${loadMoreClicks ? `, 추가 묶음 ${loadMoreClicks}회` : ""}.`
        : "YouTube 댓글을 찾지 못했습니다.",
      account_label: accountContext?.accountLabel || null,
      items,
    };
  };

  function collectYouTubeCommentStep(accountLabel, resetTop) {
    ...
    const parseRow = (row, button) => {
      ...
      return {
        ...
        metadata: {
          ...
          extractor_version: "4.0.0-comment-stepwise",
    ...
    const doc = document.scrollingElement || document.documentElement;
    const scrollCandidates = () => [doc, document.documentElement, document.body, ...document.querySelectorAll("main,c-wiz,section,div")]
      .filter(Boolean)
    ...
    const loadMoreControls = [...document.querySelectorAll("button,[role='button'],a")]
    ...
    const actionable = loadMoreControls.find(({node}) => (
    ...
    let moved = false;
    const target = scrollCandidates()[0] || doc;
    if (!clickedLoadMore && !loading) {
      const before = target.scrollTop || 0;
      const bottom = Math.max(0, target.scrollHeight - target.clientHeight);
      const amount = Math.max(620, Math.floor((target.clientHeight || innerHeight) * 0.82));
    ...
    return {
      ...
      load_more_present: loadMoreControls.length > 0,
      loading,
      moved,
      end_reached: endMarker && loadMoreControls.length === 0,
      signature: `${Math.round(target.scrollTop || 0)}:${Math.round(target.scrollHeight || 0)}:${items.length}`,
    };
  }
})();
