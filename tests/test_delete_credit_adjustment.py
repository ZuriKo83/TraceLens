...
def test_admin_adjustment_accepts_negative_amounts_safely() -> None:
    ...
    assert '@router.post("/admin/delete-credits/adjust")' in source
...
def test_admin_form_explains_and_confirms_credit_recovery() -> None:
    ...
    assert 'action="/admin/delete-credits/adjust"' in template
...
def test_adjustment_route_is_dispatched_to_account_tools_app() -> None:
    ...
    assert "account_tools_app.include_router(delete_credit_adjustment_router)" in middleware
    assert '"/admin/delete-credits/adjust"' in middleware
